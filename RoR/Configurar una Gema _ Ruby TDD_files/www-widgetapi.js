...
closed:b,Ra:b,upgrade:this.options.upgrade};return this.h=d=a()};var yh=new wh("YtIdbMeta",{ha:{databases:{V:1}},upgrade:function(a,b){b(1)&&gh(a,"databases",{keyPath:"actualName"})}});
...
c.onFetchSuccess=function(v,K){zg(p);A(v,K)}}try{t&&d.retry&&!d.qa.bypassNetworkless?(g.method="POST",!d.qa.writeThenSend&&Q("nwl_send_fast_on_unload")?pi(q,g):ji(q,g)):(g.method="POST",g.postParams||(g.postParams={}),xf(q,g))}catch(v){if("InvalidAccessError"==v.name)p&&(zg(p),p=0),Me(Error("An extension is blocking network request."));
...
ba:{ReferenceError:[{regexp:/(.*) is not defined/,groups:["key"]},{regexp:/Can't find variable: (.*)/,groups:["key"]}]}}];var wi={K:[],I:[{Ba:vi,weight:500}]};function vi(a){a=a.stack;return a.includes("chrome://")||a.includes("chrome-extension://")||a.includes("moz-extension://")}
...
g.message&&g.message.match(h.kb)){e=h.weight;break a}e=u(e.I);for(f=e.next();!f.done;f=e.next())if(f=f.value,f.Ba(g)){e=f.weight;break a}e=1}g.sampleWeight=e;e=g;g=u(ui);for(f=g.next();!f.done;f=g.next())if(f=f.value,f.ba[e.name])for(q=u(f.ba[e.name]),h=q.next();!h.done;h=q.next())if(l=h.value,h=e.message.match(l.regexp)){e.params["params.error.original"]=h[0];q=l.groups;l={};for(m=0;m<q.length;m++)l[q[m]]=h[m+1],e.params["params.error."+q[m]]=h[m+1];e.message=f.ga(l);break}e.params||(e.params={});
